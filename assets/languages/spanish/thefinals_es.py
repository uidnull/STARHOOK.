...
class StarBot(threading.Thread):
    def __init__(self, queue, keybind_type, keybind, fov, shooting_rate, fps):
        super().__init__()
        self.queue = queue
        self.keybind = KEY_MAP.get(keybind)
        self.base_fov = fov
        self.hsv_ranges = [
            (np.array(COLOR_HSV_MAP["ROSA"][0], dtype=np.uint8),
             np.array(COLOR_HSV_MAP["ROSA"][1], dtype=np.uint8)),
            (np.array(COLOR_HSV_MAP["MORADO"][0], dtype=np.uint8),
             np.array(COLOR_HSV_MAP["MORADO"][1], dtype=np.uint8)),
            (np.array(COLOR_HSV_MAP["NARANJA"][0], dtype=np.uint8),
             np.array(COLOR_HSV_MAP["NARANJA"][1], dtype=np.uint8)),
        ]
...
def editar_config():
    ...
    print(f"\nColor fijo: MULTICOLOR (ROSA + MORADO + NARANJA)")
